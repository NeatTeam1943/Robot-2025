...
public class Tag extends Command {
    ...
    public void execute() {
        ...
        if (!results.targetingResults.valid || results.targetingResults.targets_Fiducials.length == 0) {
            ...
            SmartDashboard.putString("Status", "Searching for tag...");
            return;
        }
        ...
        double angularSpeed = Math.copySign(
                Math.min(Math.abs(angleError * 1.5), Constants.Swerve.kMaxAngularVelocity),
                angleError);
    ...
    private double getAprilTagHeight(int tagID) {
        return Constants.VisionConstants.aprilTagHeights.getOrDefault(tagID,
