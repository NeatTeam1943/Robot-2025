...
public class Tag extends Command {
    ...
    public void initialize() {
        // Turn on the Limelight LEDs for vision
        LimelightHelpers.setLEDMode_ForceOn(Constants.VisionConstants.limelightName);
        LimelightHelpers.setPipelineIndex(Constants.VisionConstants.limelightName, 0);
        configureLimelight();
    }
    ...
    public void execute() {
        if (!active) {
            ...
            SmartDashboard.putString("Status", "AprilTag Follow inactive");
            return;
        }
        ...
        LimelightTarget_Fiducial closestTag = selectClosestTag(results.targets_Fiducials);
        ...
        double linearSpeed = Math.min(distance * 0.3, Constants.Swerve.kMaxSpeed);
    ...
    private void configureLimelight() {
        ...
        LimelightHelpers.setCameraPose_RobotSpace(
                ...
                Constants.VisionConstants.cameraXOffset,
                Constants.VisionConstants.cameraYOffset,
    ...
    private Pose2d getAprilTagPose(int tagID) {
        if (tagID >= 1 && tagID <= Constants.VisionConstants.FieldConstants.aprilTagPoses.length) {
            return Constants.VisionConstants.FieldConstants.aprilTagPoses[tagID - 1];
        }
        return null;
    }

    private double getAprilTagHeight(int tagID) {
        return Constants.VisionConstants.aprilTagHeights.getOrDefault(
                tagID,
    ...
    private boolean willCollideWithFieldElement(ChassisSpeeds chassisSpeeds, Pose2d targetPose) {
        Pose2d currentPose = m_swerve.getPose();
        double robotWidth = Constants.Swerve.kTrackWidth;
        ...
        double coralRadius = Constants.VisionConstants.FieldConstants.kCoralStationRadius;
        double reefRadius = Constants.VisionConstants.FieldConstants.kReefRadius;
        ...
        if (predictedPos.getX() < buffer ||
                predictedPos.getX() > Constants.VisionConstants.FieldConstants.kFieldLength - buffer ||
                predictedPos.getY() < buffer ||
                predictedPos.getY() > Constants.VisionConstants.FieldConstants.kFieldWidth - buffer) {
            return true;
        }
        return false;
    }
