...
public class RobotContainer {
  ...
  private void configureDefaultCommands() {
    ...
    m_AlgeaRotatorAxis.setDefaultCommand(
        new AlgeaRotatorAxisCommand(m_AlgeaRotatorAxis, m_MechController));
  ...
  private void autoSelector() {
    ...
    autoChooserTesting.addOption(
        "Rotation and then forword", new PathPlannerAuto("RotThenForword"));
    ...
    autoChooserTesting.addOption(
        "Checking OffSet (Be readdy! HellHole)", new PathPlannerAuto("OffSet"));
    ...
    // Middel To Up
    autoChooserGame.addOption(
        "RunAwayFromMiddelToUp", new PathPlannerAuto("RunAwayFromMiddelToUp"));
    autoChooserGame.addOption("MaxL1UpFromMiddel", new PathPlannerAuto("MaxL1UpFromMiddel"));
    autoChooserGame.addOption(
        "OneCoralFromMiddelToUp", new PathPlannerAuto("OneCoralFromMiddelToUp"));
    autoChooserGame.addOption(
        "BestCoralUpFromMiddel", new PathPlannerAuto("BestCoralUpFromMiddel"));

    // Middel To Down
    autoChooserGame.addOption(
        "RunAwayFromMiddelToDown", new PathPlannerAuto("RunAwayFromMiddelToDown"));
    autoChooserGame.addOption("MaxL1DownFromMiddel", new PathPlannerAuto("MaxL1DownFromMiddel"));
    autoChooserGame.addOption(
        "OneCoralFromMiddelToDown", new PathPlannerAuto("OneCoralFromMiddelToDown"));
    autoChooserGame.addOption(
        "BestCoralDownFromMiddel", new PathPlannerAuto("BestCoralDownFromMiddel"));
  ...
  public RobotContainer() {
    ...
    m_MechController =
        new CommandXboxController(Constants.OperatorConstants.kMechanisemControllerPort);
    m_DriveController =
    ...
    NamedCommands.registerCommand(
        "Elevator L1", new ResetElevatorCommand(m_Elevator, m_LedController));
    NamedCommands.registerCommand(
        "Elevator L2",
  ...
  private void configureButtonBindings() {
    ...
    m_MechController
        .x()
        .onTrue(
            new CoralCommand(m_Coral, m_LedController)
                .andThen(new ResetElevatorCommand(m_Elevator, m_LedController)));

    m_MechController.povDown().onTrue(new ResetElevatorCommand(m_Elevator, m_LedController));
    m_MechController
        .povLeft()
    ...
    m_MechController
        .a()
        .onTrue(new InstantCommand(() -> m_Elevator.resetEncoderValue(), m_Elevator));
